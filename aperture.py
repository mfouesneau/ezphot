...
__all__ = ["CircularAperture", "CircularAnnulus", "CircularSector",
...
class CircularAperture(Aperture):
    ...
    def encloses(self, xx, yy):
        ...
        in_aper : `~numpy.ndarray` (bool)
            ...
            aperture.
        """
        return np.add(np.multiply(xx, xx) , np.multiply( yy , yy)) < self.r * self.r
...
class CircularSector(Aperture):
    ...
    def __init__(self, r_in, r_out, theta1, theta2):
        if not (r_out > r_in):
            raise ValueError('r_out must be greater than r_in')
        if not (r_in >= 0.):
            raise ValueError('r_in must be non-negative')
    ...
    def extent(self):
        ...
        x_min, x_max, y_min, y_max: float
        """
	t = np.linspace(self.theta1, self.theta2, 40)
	x = np.hstack([self.r_out * np.cos(t), self.r_in * np.cos(t) ])
	y = np.hstack([self.r_out * np.sin(t), self.r_in * np.sin(t) ])
    ...
    def area(self):
        ...
        area : float
            Area in pixels enclosed by aperture.
        """
        return 0.5 * (self.r_out ** 2 - self.r_in ** 2) * abs(self.theta2-self.theta1)
